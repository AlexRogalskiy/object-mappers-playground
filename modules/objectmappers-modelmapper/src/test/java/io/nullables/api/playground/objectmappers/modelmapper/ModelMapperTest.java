 ...
 */
package io.nullables.api.playground.objectmappers.modelmapper;
...
class ModelMapperTest {
	...
	@BeforeEach
	void setUp() {
		this.modelMapper = ModelMapperBuilder.newBuilder().withPropertyMap(new AddressDtoToAddressEntityPropertyMap())
				.withPropertyMap(new DeliveryDtoToDeliveryEntityPropertyMap()).build();
	}
	...
	void testCheckDeliveryDtoConversion(@Nonnull final DeliveryDto source) {
		// when
		final DeliveryEntity target = this.modelMapper.map(source, DeliveryEntity.class);
