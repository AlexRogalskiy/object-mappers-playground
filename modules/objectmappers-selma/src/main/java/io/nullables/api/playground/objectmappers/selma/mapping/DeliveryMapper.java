...
@Mapper(withIgnoreMissing = IgnoreMissing.ALL, withCollectionStrategy = ALLOW_GETTER,
                withCustomFields = {@Field(value = {"id", "id"}, withCustom = StringToUuidMapper.class),
                                ...
                                @Field({"gid", "gid"}), @Field({"createdAt", "createdAt"}),
                                @Field({"updatedAt", "updatedAt"}),
                                @Field(value = {"shippableDue", "shippableDue"},
                                                withCustom = StringToLocalDateTimeMapper.class),
                                @Field({"balance", "balance"}),
                                @Field(value = {"discount", "discount"}, withCustom = StringToBigDecimalMapper.class),
                                @Field({"status", "status"}), @Field({"addresses", "addresses"}),
