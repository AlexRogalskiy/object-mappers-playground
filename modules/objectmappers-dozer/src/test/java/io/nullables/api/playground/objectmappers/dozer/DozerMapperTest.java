 ...
 */
package io.nullables.api.playground.objectmappers.dozer;
...
class DozerMapperTest {
	...
	@BeforeEach
	void before() {
		this.mapper = DozerMapperConfiguration.newBuilder().configureMapper("mappings/config_mapping.xml",
				"mappings/address_mapping.xml", "mappings/delivery_mapping.xml").build();
	}
	...
	void testCheckDeliveryDtoConversion(@Nonnull final DeliveryDto source) {
		// when
		final DeliveryEntity target = this.mapper.map(source, DeliveryEntity.class);
