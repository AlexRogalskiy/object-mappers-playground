...
public class ReMapperConfiguration {
	...
	public Mapper<DeliveryDto, DeliveryEntity> deliveryMapper() {
		return Mapping.from(DeliveryDto.class).to(DeliveryEntity.class).useMapper(this.addressMapper())
				.replace(DeliveryDto::getCodes, DeliveryEntity::getCodes).with(StringUtils::convertToIntegerArray)
				.replace(DeliveryDto::getId, DeliveryEntity::getId).with(StringUtils::convertToUuid)
				.replace(DeliveryDto::getShippableDue, DeliveryEntity::getShippableDue)
				.with(DateUtils::convertToLocalDateTime).replace(DeliveryDto::getDiscount, DeliveryEntity::getDiscount)
