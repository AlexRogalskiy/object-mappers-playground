...
public class MooMapper {
    ...
    public DeliveryEntity translate(final DeliveryDto value) {
        return Translate.to(DeliveryEntity.class)
            ...
            .withVariable("gid", value.getGid())
            .withVariable("createdAt", value.getCreatedAt())
            ...
            .withVariable(
                "addresses",
                Optional.ofNullable(value.getAddresses())
                    .orElseGet(Collections::emptyList)
                    .stream()
                    .map(this::translate)
                    .collect(Collectors.toList())
            )
            .withVariable("codes", StringUtils.convertToIntegerArray(value.getCodes()))
            .from(value);
    }
